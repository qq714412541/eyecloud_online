...
class Lesion:
    ...
    def lesion_locate(self, image_path, spot, disc_radius):
        ...
        mic_name = 'mic_' + f_name
        mic_output_path = os.path.join(self.config['output_dir'], mic_name)
        ...
        hem_distances = []
        mic_distances = []
        lesion_list = list(zip(boxes[0], scores[0], labels[0]))
        args = nms.non_max_suppression(boxes[0], scores[0], labels[0], [1, 2])
        for arg in args:
            box, score, label = lesion_list[arg]
            ...
            elif label == self.config['prefix']['mic']:
                mic_distances.append(distance)
                draw_box(draw_mic, b, color=[255, 255, 255])                
        ...
        re_mic = generate_result('mic', mic_distances, mic_output_path, draw_mic)

        return (re_exu, re_hem, re_mic, lesion_result)
