...
def generate(config, save_path):
    ...
    c1.drawCentredString(215, h-340, config['dr'])
    # image quality content
    c1.drawCentredString(304, h-282, config['image-point'])
    c1.drawCentredString(333, h-282, config['image-quality'])
    c1.drawCentredString(441, h-282, config['vessel-point'])
    c1.drawCentredString(471, h-282, config['vessel-quality'])
    c1.drawCentredString(277, h-299, config['distance'])
    c1.drawCentredString(168, h-316, config['angle'])
    if config['dr'] == u'有':
        c1.drawCentredString(262, h-340, config['stage'])
        c1.drawCentredString(391, h-340, config['bleed'])
        c1.drawCentredString(151, h-358, config['1-bleed'])
        c1.drawCentredString(221, h-357, config['exudation'])
        c1.drawCentredString(378, h-358, config['1-exudation'])
    if config['vessel_analysis']:
        offset = 0
        if config['dr'] == u'有':
            offset = 18
        c1.drawString(241, h-364-offset, config['2-length'])
        c1.drawString(399, h-364-offset, config['2-length_compare'])
        c1.drawString(117, h-381-offset, config['2-density'])
        c1.drawString(245, h-381-offset, config['2-density_compare'])
        c1.drawString(373, h-381-offset, config['2-diameter'])
        c1.drawString(143, h-397-offset, config['2-diameter_compare'])
    c1.drawImage(config['origin_image'], 153, h-658, width=2880//10, height=2136//10)
    c1.save()
    ...
    # Add content
    c2.drawImage(config['macular_image'], 100, h-245, width=2880//18, height=2136//18)
    ...
    if config['dr'] == u'有':
        c2.drawImage(config['bleed_image'], 113, h-522, width=2880//18, height=2136//18)
        c2.drawImage(config['bleed_histogram'], 307, h-532, width=187, height=140)
        c2.drawImage(config['exudation_image'], 113, h-703, width=2880//18, height=2136//18)
        c2.drawImage(config['exudation_histogram'], 307, h-713, width=187, height=140)
    c2.save()
    ...
    if config['vessel_analysis']:
        ...
        # Add content
        c3.drawImage(config['retinal_vessel_image'], 113, h-273, width=2880//18, height=2136//18)
        c3.drawImage(config['quadrant_segmentation_image'], 320, h-273, width=2880//18, height=2136//18)
        ...
        c3.drawImage(config['a-patient_length_histogram'], 198, h-588, width=150, height=116)
        ...
        c3.drawImage(config['a-patient_diameter_histogram'], 198, h-729, width=150, height=116)
...
if __name__ == '__main__':
    ...
    config['2-length'] = '100'
    config['2-length_compare'] = '+10'
    config['2-density'] = '20'
    config['2-density_compare'] = '-2'
    config['2-diameter'] = '50'
    config['2-diameter_compare'] = '+5'
